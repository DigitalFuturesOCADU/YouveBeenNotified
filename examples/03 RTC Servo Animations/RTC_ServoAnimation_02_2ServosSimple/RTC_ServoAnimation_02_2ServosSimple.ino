...
int lastMinute = -1;
int debugCounter = 0;  // Counter for debug output
...
void setup() 
{
  ...
  Serial.println("RTCservo Two Servos Example");
  Serial.println("Servo 1: Alternates animations each minute");
  Serial.println("Servo 2: Continuously oscillates between 0-180 degrees");
  Serial.print("Animation Speed: ");
  Serial.println(playSpeed);
  Serial.print("Minute Offset: ");
  Serial.println(minuteOffset);
  
  // Start both animations immediately
  // Even with minute offset, we want to start one of the animations right away
  if ((minuteOffset % 2) == 0) {
    Serial.println("Starting with Wave animation on Servo 1");
    notifier1.playAnimation(anim1Name, LOOP);
  } else {
    Serial.println("Starting with Sweep animation on Servo 1");
    notifier1.playAnimation(anim2Name, LOOP);
  }
  
  Serial.println("Starting Oscillate animation on Servo 2");
...
void loop() 
{
  ...
  // Debug output every ~2 seconds to verify animations are running
  debugCounter++;
  if (debugCounter >= 100) {
    debugCounter = 0;
    Serial.print("Animation status - Servo 1: ");
    Serial.print(notifier1.getCurrentAnimationName());
    Serial.print(" (");
    Serial.print(notifier1.getValue());
    Serial.print("°), Servo 2: ");
    Serial.print(notifier2.getCurrentAnimationName());
    Serial.print(" (");
    Serial.print(notifier2.getValue());
    Serial.println("°)");
  }
